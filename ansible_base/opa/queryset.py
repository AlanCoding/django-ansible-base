...
def filter_queryset_for_user(queryset, user, action):
    ...
    Resolves the user's policy IDs and sends them to OPA, which looks up
    the policy definitions and returns resolved clauses for queryset filtering.
...
def user_can_access_obj(user, obj, action, related=None):
    ...
    Uses tier 2 OPA evaluation: sends the user's policy IDs and the
    object's attributes to OPA for a concrete boolean answer. Optionally
...
def check_object_access(user, resource, action, obj, related=None):
    ...
    Resolves the user's policy IDs and sends them along with the object
    attributes to OPA for evaluation. OPA looks up the policy definitions
    from its stored data and evaluates.
    ...
    from ansible_base.opa.evaluator import _extract_obj_attrs, get_user_policy_ids

    obj_attrs = _extract_obj_attrs(obj, resource)
    policy_ids = get_user_policy_ids(user)
...
def get_opa_scope(user, resource, action):
    ...
    Resolves the user's policy IDs and sends them to OPA. OPA looks up
    the referenced policies from its stored data, filters by resource/action,
    and returns resolved clauses.
    ...
    """
    from ansible_base.opa.client import OPAClient
    from ansible_base.opa.evaluator import get_user_policy_ids
